...
# Motifs
motif_human  <- c("CIS_BP_2.00_Homo_sapiens", "CTCFBSDB_PWM_corrected", "HOCOMOCOv11_core_HUMAN_mono_meme_format", "JASPAR2022_CORE_vertebrates_non_redundant_v2", "jolma2013_corrected", "SwissRegulon_human_and_mouse")
motif_mouse  <- c("CIS_BP_2.00_Mus_musculus", "CTCFBSDB_PWM_corrected", "HOCOMOCOv11_core_MOUSE_mono_meme_format", "JASPAR2022_CORE_vertebrates_non_redundant_v2", "jolma2013_corrected", "SwissRegulon_human_and_mouse")
# Genomes
...
combine_fimo <- function(motif = "SwissRegulon_human_and_mouse", genome_id = "T2T", chromosomes = chromosome_human) {
  ...
  # Summary
  # Width
  summary_gr <- summary(width(eval(parse(text = ctcfGR_to_save))))
  # All summary
  summary_to_save <- data.frame(File = ctcfGR_to_save,
                                N = length(width(eval(parse(text = ctcfGR_to_save)))), 
                                Min = as.numeric(summary_gr["Min."]), 
                                Median = as.numeric(summary_gr["Median"]), 
                                Mean = as.numeric(summary_gr["Mean"]), 
                                Max = as.numeric(summary_gr["Max."]))
  fileNameLog <- file.path(dir_data, "log_PWMs.csv")
  write_csv(summary_to_save, fileNameLog, append = TRUE)
}
```
